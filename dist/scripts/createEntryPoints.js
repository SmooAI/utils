...
var createEntryPoints_exports = {};
__export(createEntryPoints_exports, {
  default: () => UpdateTsupConfig
});
module.exports = __toCommonJS(createEntryPoints_exports);
...
// src/scripts/createEntryPoints.ts
var UpdateTsupConfig = class _UpdateTsupConfig extends import_core.Command {
  static description = "Update tsup.config.ts entry field based on provided patterns";
  static flags = {
    include: import_core.Flags.string({
      name: "include",
      char: "i",
    ...
    ignore: import_core.Flags.string({
      name: "ignore",
      char: "x",
  ...
  async run() {
    const { flags } = await this.parse(_UpdateTsupConfig);
    ...
    try {
      ...
      let tsupFile = (0, import_node_fs.readFileSync)("tsup.config.ts", "utf-8");
      tsupFile = tsupFile.replace(/entry: \[[^\]]*\],/, `entry: [${files.map((file) => `'${file}'`).join(",")}],`);
      (0, import_node_fs.writeFileSync)("tsup.config.ts", tsupFile);
