...
- afb59b6: Migrate build tooling from tsup to tsdown — faster, oxc-based, drop-in replacement. The package now publishes `.cjs`/`.mjs`/`.d.cts`/`.d.mts` outputs (instead of `.js`/`.mjs`/`.d.ts`); the `exports` map is updated accordingly, so subpath imports continue to resolve transparently. No public API change.

## 1.3.3

### Patch Changes

- da5ae70: SMOODEV-928: Bump `@smooai/logger` to `^4.1.4` to pick up the ESM `__filename` TDZ fix. Prior runtime range pulled logger 3.x, which crashed under tsx-run consumers in the smooai monorepo due to const TDZ on `__dirname`/`__filename`.
